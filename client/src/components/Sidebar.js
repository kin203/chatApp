...
const Sidebar = () => {
    ...
    const [allUser, setAllUser] = useState([]);
    const [openSearchUser, setOpenSearchUser] = useState(false);
    const [showLogoutConfirm, setShowLogoutConfirm] = useState(false); // Trạng thái để hiển thị modal logout
    const socketConnection = useSelector((state) => state?.user?.socketConnection);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    ...
    const handleLogout = () => {
      ...
      navigate("/email");
      localStorage.clear();
      setShowLogoutConfirm(false); // Đóng modal sau khi đăng xuất
    };

    const handleCancelLogout = () => {
      setShowLogoutConfirm(false); // Đóng modal nếu không muốn đăng xuất
    };
